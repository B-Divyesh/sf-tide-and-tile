...
type SavedData = { muted?: boolean; best?: Record<string, number>; current?: SavedGame };
...
function writeData() {
  try {
    localStorage.setItem(storageKey(), JSON.stringify({ muted, best, current: { seed: board.seed, name: boardName, guided, turns, state, rotations: board.tiles.map(tile => tile.rotation) } } satisfies SavedData));
  } catch { /* The game remains playable when storage is unavailable. */ }
}
function loadBoard(seed: string, name: string, isGuided: boolean) { board = makeBoard(seed, isGuided); boardName = name; guided = isGuided; turns = 0; state = 'playing'; selected = 0; writeData(); }
function restoreOrStart(seed: string, name: string, isGuided: boolean) {
  const saved = readData(); muted = Boolean(saved.muted); best = saved.best || {};
  if (saved.current?.rotations.length === 16) {
    board = makeBoard(saved.current.seed, saved.current.guided); boardName = saved.current.name; guided = saved.current.guided;
    board.tiles = board.tiles.map((tile, index) => ({ ...tile, rotation: saved.current!.rotations[index] }));
    turns = saved.current.turns; state = saved.current.state; return;
  }
  loadBoard(seed, name, isGuided);
}
...
function tipText() {
  ...
  if (tutorialStep === 1) return `Turn four misplaced tiles. Every channel must form one dock-to-harbor route. Seed ${board.seed}.`;
  if (tutorialStep === 2) return `Every water end must meet its neighboring channel. Seed ${board.seed}.`;
...
function appPage() {
  return `${nav()}${demo ? `<aside class="demo-banner" aria-label="Demo status"><b>Demo — sample data, nothing is saved</b><button id="reset-demo">Reset demo</button><button id="start-real">Start for real</button></aside>` : ''}<main id="main" tabindex="-1"><section class="hero"><div><p class="eyebrow">DAILY HARBOR PUZZLE</p><h1>Make today’s harbor route</h1><p class="intro">For casual players who want a calm puzzle break with clear rules.</p><div class="actions"><a class="button primary" href="/demo" data-link>Try it with sample data</a><span>Loads a guided board. It does not change your daily progress.</span></div><ul class="facts"><li>Free to play</li><li>Works offline after the first visit</li><li>Progress stays in this browser</li></ul></div><figure><img src="/harbor-table.webp" width="768" height="512" alt="A harbor puzzle table with blue water tiles and a chart." loading="eager" fetchpriority="high"><figcaption>Four tiles need one turn in the sample.</figcaption></figure></section><section class="game-zone" aria-labelledby="game-title"><div class="game-heading"><div><p class="eyebrow">${demo ? 'SAMPLE BOARD' : 'CURRENT BOARD'}</p><h2 id="game-title">${escapeHtml(boardName)}</h2></div><div class="statbar"><span>Turns <b id="turns">${turns}</b> / ${board.maxTurns}</span><span>Fewest <b id="par">${board.par}</b></span><button id="sound" aria-pressed="${muted}">${muted ? 'Sound off' : 'Sound on'}</button></div></div><p id="tip" class="tip" aria-live="polite">${tipText()}</p><div id="board" class="board" role="group" aria-label="Harbor tile board"></div><div class="game-actions"><button id="restart">Restart this board</button><button id="help">Show the next rule</button><button id="share" ${state === 'won' ? '' : 'disabled'}>Copy result</button></div><p id="result" class="result" aria-live="polite">${state === 'won' ? `Route complete. ${medal()}.` : ''}</p>${endScreen()}</section><section id="archive" class="archive"><div><p class="eyebrow">THREE SEEDED ROUTES</p><h2>Archive boards</h2><p>Each route has a different connected layout.</p></div><div class="archive-buttons"><button data-archive="Dock lesson" data-seed="archive-dock">Dock lesson <small>Guided route</small></button><button data-archive="Breakwater bend" data-seed="archive-breakwater">Breakwater bend <small>Corner practice</small></button><button data-archive="Harbor circuit" data-seed="archive-circuit">Harbor circuit <small>Full scramble</small></button></div></section><section class="how"><h2>How to play Tide & Tile</h2><ol><li><b>Turn</b> a tile by tapping it or pressing Enter.</li><li><b>Join</b> every channel into one continuous route.</li><li><b>Finish</b> near the fewest turns to earn a medal.</li></ol><p><b>What it does not do:</b> there are no timers, lives, accounts, or leaderboards. The turn limit gives each route a clear finish.</p></section></main>${footer()}`;
}
...
function renderBoard() {
  ...
  board.tiles.forEach((tile, index) => {
    ...
    button.className = `tile rotation-${tile.rotation}${index === board.dock.index ? ' dock' : ''}${index === board.harbor.index ? ' harbor' : ''}`; button.dataset.i = String(index); button.dataset.needed = String(needed);
...
function bind() {
  document.querySelectorAll<HTMLAnchorElement>('[data-link]').forEach(link => link.addEventListener('click', event => { event.preventDefault(); history.pushState({}, '', link.href); renderRoute(true); }));
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-archive]').forEach(button => button.addEventListener('click', () => { loadBoard(button.dataset.seed!, button.dataset.archive!, button.dataset.archive === 'Dock lesson'); renderGame(); document.querySelector('#game-title')?.scrollIntoView({ behavior: matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth' }); }));
}
...
function renderRoute(moveFocus = false) {
  ...
  if (route === '/privacy' || route === '/terms') app.innerHTML = documentPage(route.slice(1) as 'privacy' | 'terms');
    ...
    demo = route === '/demo';
    if (demo) { tutorialStep = 1; restoreOrStart('sample-harbor', 'Sample harbor', true); }
    else {
      const visitsKey = 'tide:tide-and-tile:visits', visits = Number(localStorage.getItem(visitsKey) || '0'); tutorialStep = Math.min(2, visits + 1);
      if (!localStorage.getItem(storageKey())) localStorage.setItem(visitsKey, String(visits + 1));
      restoreOrStart(dailySeed(), 'Today’s tide', visits === 0);
    }
    renderGame();
